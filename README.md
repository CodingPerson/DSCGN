...
We provide four ```get_data.sh``` scripts for downloading the datasets used in the paper under ```datasets``` and four training bash scripts [```profession-wiki-page.sh```](professio-wiki-page.sh) for running the model on the four datasets.
...
* ```train_epochs```: They control how many epochs to train the model 
