...
from transformers import BertTokenizer, AdamW, get_linear_schedule_with_warmup, RobertaTokenizer
import numpy as np
import os
import shutil
import sys
...
class LOTClassTrainer(object):
    ...
    def __init__(self, args):
        ...
        self.mcp_loss = nn.CrossEntropyLoss(reduction='none')
    ...
    def make_mcp_dataloader(self, rank, data_dict, batch_size):
        if "labels" in data_dict:
            dataset = TensorDataset(data_dict["input_ids"], data_dict["attention_masks"], data_dict["labels"],data_dict['weight'])
        else:
            dataset = TensorDataset(data_dict["input_ids"], data_dict["attention_masks"],data_dict['weight'])
        sampler = DistributedSampler(dataset, num_replicas=self.world_size, rank=rank)
        dataset_loader = DataLoader(dataset, sampler=sampler, batch_size=batch_size, shuffle=False)
        return dataset_loader
    ...
    def prepare_mcp_dist(self, rank, top_pred_num=50, match_threshold=10, loader_name="mcp_train.pt"):
        ...
        try:
            for batch in wrap_train_dataset_loader:
                with torch.no_grad():
                    ...
                    for i, category_vocab in self.category_vocab.items():
                        ...
                        weights_count = torch.div(match_count.float(),len(category_vocab))
            ...
            all_input_weight = torch.cat(all_input_weight,dim=0)
            save_dict = {
    ...
    def prepare_mcp(self, top_pred_num=50, match_threshold=2, loader_name="mcp_train.pt"):
        ...
        if os.path.exists(loader_file):
            ...
            all_input_weight = torch.cat([res["all_input_weight"] for res in gather_res], dim=0)
            ...
            for i in range(epochs):
               ...
               # web_dataset_loader = tqdm(web_dataset_loader) if rank == 0 else web_dataset_loader
                ...
                for j, batch in enumerate(wrap_mcp_dataset_loader):
                    ...
                    loss = self.GCE_loss(logits, labels) / self.accum_steps
                    #chenhu
                    #按权重对loss进行加权
                    # softmax = nn.Softmax()
                ...
                if rank == 0:
                    ...
                    self.write_no_split_results(loader_name="mcp_model.pt", out_file="mcp_out.txt")
            ...
            for batch in dataset_loader:
                with torch.no_grad():
                    ...
                    if return_type == "data":
                        ...
                        #chenhu
                        pred_labels.append(nn.Softmax(dim=-1)(logits).cpu())
                        #pred_labels.append(torch.argmax(logits, dim=-1).cpu())
            if return_type == "data":
                all_input_ids = torch.cat(all_input_ids, dim=0)
                all_input_mask = torch.cat(all_input_mask, dim=0)
    ...
    def write_no_split_results(self, loader_name="final_model.pt", out_file="out.txt"):
        truths = open(os.path.join(self.dataset_dir, self.args.gold_label_file))
        #chenhu
        #map_file = open(os.path.join(self.dataset_dir, self.args.map_file))
        gold_labels = [str(label.strip()) for label in truths.readlines()]
